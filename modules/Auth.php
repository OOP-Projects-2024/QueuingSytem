...
class Authentication{
    ...
    private function generatePayload($id, $username){
        $payload = [
            "uid" => $id,
            "uc" => $username,
            "email" => "202211660@gordoncollege.edu.ph",
            "date" => date_create(),
           ...
           if($stmt->rowCount() > 0){
            $result = $stmt->fetchAll()[0];
                if($this->isSamePassword($password, $result['password'])){
                    ...
                    $payload = array("chefsid"=>$result['userid'], "username"=>$result['username'], "token"=>$token_arr[2]);
                }
                else{
                    $code = 401;
                    $payload = null;
    ...
    public function addAccount($body){
        ...
        try{
            $sqlString = "INSERT INTO accounts_tbl(userid, username, password) VALUES (?,?,?)";
            $sql = $this->pdo->prepare($sqlString);
            $sql->execute($values);
